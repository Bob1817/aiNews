...
export const envSchema = z.object({
  ...
  DB_MAX_CONNECTIONS: z.preprocess(Number, z.number()).default(10),
  DB_TIMEOUT: z.preprocess(Number, z.number()).default(10000),

  // API 配置
  API_BASE_URL: z.string().default('http://localhost:3001'),
  API_TIMEOUT: z.preprocess(Number, z.number()).default(30000),
  RATE_LIMIT_WINDOW_MS: z.preprocess(Number, z.number()).default(900000), // 15 minutes
  ...
  RATE_LIMIT_ENABLED: z.preprocess((val) => val === 'true', z.boolean()).default(true),

  // 日志配置
  LOG_LEVEL: z.enum(['error', 'warn', 'info', 'debug']).default('info'),
  LOG_FORMAT: z.enum(['json', 'text']).default('text'),
  ENABLE_FILE_LOGGING: z.preprocess((val) => val === 'true', z.boolean()).default(false),
  ...
  NEWS_API_TIMEOUT: z.preprocess(Number, z.number()).default(12000),
  NEWS_API_MAX_RETRIES: z.preprocess(Number, z.number()).default(3),

  // AI 服务配置
  AI_DEFAULT_MODEL: z.string().default('gpt-3.5-turbo'),
  AI_MAX_TOKENS: z.preprocess(Number, z.number()).default(2000),
  AI_TEMPERATURE: z.preprocess(Number, z.number()).default(0.7),
  AI_TIMEOUT: z.preprocess(Number, z.number()).default(30000),

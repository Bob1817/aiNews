...
exports.NYTAPIAdapter = exports.GuardianAPIAdapter = exports.NewsAPIAdapter = void 0;
const news_api_interface_1 = require("./news-api.interface");
class NewsAPIAdapter extends news_api_interface_1.NewsAdapter {
    getName() {
        return 'NewsAPI';
    }
    async fetchNews(userProfile) {
        ...
        try {
            ...
            if (!response.ok) {
                const remoteMessage = data && typeof data === 'object' && 'message' in data ? `: ${data.message}` : '';
                throw new Error(`NewsAPI 请求失败 ${response.status}${remoteMessage}`);
            }
            if (data.articles && Array.isArray(data.articles)) {
                // 过滤掉标题为 [Removed] 的新闻，并优先选择中文来源
                const validArticles = data.articles.filter((article) => article.title && article.title !== '[Removed]' && article.title !== '');
                // 优先选择中文来源的新闻
                const chineseSources = ['新浪', '腾讯', '网易', '搜狐', '人民日报', '新华社', '央视新闻', '中国新闻网'];
                const chineseArticles = validArticles.filter((article) => chineseSources.some(source => article.source?.name?.includes(source) || article.title?.includes(source)));
                // 如果中文新闻不够，使用其他新闻
...
class GuardianAPIAdapter extends news_api_interface_1.NewsAdapter {
    getName() {
        return 'The Guardian';
    }
    async fetchNews(userProfile) {
...
class NYTAPIAdapter extends news_api_interface_1.NewsAdapter {
    getName() {
    ...
    async fetchNews(userProfile) {
        ...
        try {
            ...
            const { response, data } = await this.fetchJsonWithTimeout(requestUrl, this.config.timeout);
            if (!response.ok) {
                const remoteMessage = data && typeof data === 'object' && 'fault' in data && data.fault && 'faultstring' in data.fault ? `: ${data.fault.faultstring}` : '';
...
//# sourceMappingURL=news-adapters.js.map
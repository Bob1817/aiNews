...
exports.envSchema = zod_1.z.object({
    ...
    DB_MAX_CONNECTIONS: zod_1.z.string().transform(Number).default('10'),
    DB_TIMEOUT: zod_1.z.string().transform(Number).default('10000'),
    // API 配置
    ...
    RATE_LIMIT_ENABLED: zod_1.z.string().transform(val => val.toLowerCase() === 'true').default('true'),
    // 日志配置
    LOG_LEVEL: zod_1.z.enum(['error', 'warn', 'info', 'debug']).default('info'),
    LOG_FORMAT: zod_1.z.enum(['json', 'text']).default('text'),
    ENABLE_FILE_LOGGING: zod_1.z.string().transform(val => val.toLowerCase() === 'true').default('false'),
    ...
    NEWS_API_TIMEOUT: zod_1.z.string().transform(Number).default('12000'),
    NEWS_API_MAX_RETRIES: zod_1.z.string().transform(Number).default('3'),
    // AI 服务配置
    AI_DEFAULT_MODEL: zod_1.z.string().default('gpt-3.5-turbo'),
    AI_MAX_TOKENS: zod_1.z.string().transform(Number).default('2000'),
    AI_TEMPERATURE: zod_1.z.string().transform(Number).default('0.7'),
    AI_TIMEOUT: zod_1.z.string().transform(Number).default('30000'),
    // 前端配置 (Vite 环境变量)

...
class ConfigService {
    ...
    // 保存配置
    async saveConfig(userId, configData) {
        const config = ConfigService.userConfigs.find((c) => c.userId === userId);
        if (!config) {
        ...
        if (configData.aiModels) {
            ...
            config.aiModels = configData.aiModels.map((model) => ({
                ...model,
                isActive: isFirstModel || model.id === config.aiModel.id,
            }));
        }
        const activeModel = config.aiModels.find((model) => model.isActive) ||
    ...
    // 切换 AI 模型
    async switchAIModel(userId, modelId) {
        const config = ConfigService.userConfigs.find((c) => c.userId === userId);
        if (!config) {
            throw new Error('配置不存在');
        }
    ...
    // 删除 AI 模型
    async deleteAIModel(userId, modelId) {
        const config = ConfigService.userConfigs.find((c) => c.userId === userId);
        if (!config) {
            throw new Error('配置不存在');
        }

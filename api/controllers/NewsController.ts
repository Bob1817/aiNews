...
export class NewsController {
  ...
  // 获取最近新闻
  async getRecentNews(req: Request, res: Response) {
    try {
      ...
      if (userId) {
        try {
          userProfile = await this.userService.getProfile(userId as string)
        } catch (error) {
          console.log('获取用户资料失败，使用默认设置:', error)
        }
      }
      
      const news = await this.newsService.getRecentNews(userId as string, userProfile)
      res.json(news)
    } catch (error) {
  ...
  async getSavedNews(req: Request, res: Response) {
    try {
      const { userId } = req.query
      const news = await this.savedNewsService.getSavedNews(userId as string)
      res.json(news)
    } catch (error) {
  ...
  // 手动触发新闻更新
  async updateNewsFeeds(req: Request, res: Response) {
    try {
      const { userId } = req.query
      ...
      await this.newsService.updateNewsFeeds(userId as string, userProfile)

...
function createBuiltInNewsWorkflow(): WorkflowDefinition {
...
function createBuiltInNewsDigestWorkflow(): WorkflowDefinition {
  const now = createTimestamp()
  ...
  return {
    ...
    systemInstruction:
      '你是 AI 助手中的内置新闻推送工作流。必须把用户在对话中输入的内容优先解析为 AI 爬虫关键词，并结合用户长期偏好去抓取互联网上的相关新闻，再整理成简洁可读的新闻简报。',
    steps: [
      {
...
export class WorkflowService {
  ...
  async updateWorkflow(id: string, input: Partial<WorkflowDefinition>): Promise<WorkflowDefinition> {
    ...
    const merged: WorkflowDefinition = {
      ...
      invocation: {
        ...
        aliases: (input.invocation?.aliases || existing.invocation.aliases).map((item) =>
          this.normalizeInvocation(item)
        ),
  ...
  private validateWorkflowInput(input: Partial<WorkflowDefinition>) {
    if (!input.name?.trim()) {
  ...
  private normalizeInvocation(value: string) {
